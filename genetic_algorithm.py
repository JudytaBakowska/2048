...
def genetic_algorithm(pop_size, num_generations, num_games, mutation_rate, save_results):
    ...
    for gen in range(num_generations):
        ...
        scores, max_values = evaluate_population(population, num_games)
        ...
        if save_results:
            save_to_file(file_num, gen+1, population, only_scores, max_values)
...
def play_single_game(*params):
    ...
    while not game_over:
        algorithm.create_expectimax_tree(2, board.grid, *params)
...
def save_to_file(file_num, generation, population, scores, max_tiles):
    filename = f"results{file_num}.txt"

    with open(filename, "a") as file:
        file.write(f"{generation},{population},{scores}, {max_tiles}\n")
