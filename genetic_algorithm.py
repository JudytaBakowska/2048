...
def evaluate_population(population, num_games):
    scores = []
    max_values = []
    max_dict = {2048: 0, 1024: 0, 512: 0, 256: 0, 128: 0, 64: 0, 32: 0, 16: 0, 8: 0}

    for parameters in population:
        score_sum = 0
        max_value = 0
        for _ in range(num_games):
            score_single_game, max_tile = play_single_game(*parameters)
            score_sum += score_single_game
            max_dict[max_tile] += 1
            if max_tile > max_value: max_value = max_tile

        avg_score = score_sum // num_games
        scores.append((parameters, avg_score))
        max_values.append(max_value)

    return scores, max_values, max_dict
...
def genetic_algorithm(pop_size, num_generations, num_games, mutation_rate, save_results):
    ...
    for gen in range(num_generations):
        ...
        scores, max_values, max_dict = evaluate_population(population, num_games)
        ...
        if save_results:
            append_to_result_list(result, gen + 1, population, only_scores, max_values, max_dict)
...
def play_single_game(*params):
    ...
    while not game_over:
        algorithm.create_expectimax_tree(4, board.grid, *params)
    ...
    return board.score[0], max_tile


def return_file_number():
    counter = 1
    while os.path.exists(f"results{counter}.json"):
        counter += 1

    return counter


def append_to_result_list(result, generation, population, scores, max_tiles, max_dict):
    data = {
...
def save_to_file(file_num, results):
    filename = f"results{file_num}.json"
