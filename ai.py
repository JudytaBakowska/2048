...
def create_minimax_tree(depth, board_values):
    global node_scores
    global child_list
    global node_number
    node_scores = [0 for _ in range(50000)]
    ...
    alpha_beta_pruning(1, grid, 0, depth, -math.inf, math.inf)


def alpha_beta_pruning(node, grid, parent, depth, alpha, beta):
    global node_scores
    global child_list
    global node_number
    if depth == 0:
        node_scores[node] = get_score(grid)
    ...
    if depth % 2 == 0:
        for i in range(4):
            ...
            if move_possible(grid, directions[i]):
                alpha = max(alpha, alpha_beta_pruning(node_number, swipe_grid(grid, directions[i]), node, depth-1, alpha, beta))
            if alpha >= beta:
                break
    ...
    #the turn of computer
        ...
        for i in grid_table:
            grid_table_scores.append(get_score(i))
        ...
        for i in range(4):
            ...
            node_number += 1
            child_list[node].append(node_number)
            beta = min(beta, alpha_beta_pruning(node_number, grid_table[index], node, depth-1, alpha, beta))
